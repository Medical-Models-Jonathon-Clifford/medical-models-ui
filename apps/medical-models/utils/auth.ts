...
export const {handlers, auth, signIn, signOut} = NextAuth({
    ...
    callbacks: {
        authorized({request, auth}) {
            console.log('authorized callback ran. request: %o. auth: %o.', request, auth);
            const {pathname} = request.nextUrl
            if (pathname === "/middleware-example") return !!auth
            return true
        },
        jwt({token, trigger, session, account}) {
            console.log('jwt callback ran. token: %o. trigger: %o. session: %o. account: %o.', token, trigger, session, account);
        ...
        async session({session, token}) {
            console.log('session callback ran. session: %o. token: %o.', session, token);
