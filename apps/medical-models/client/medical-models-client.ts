...
export function newDocumentWithParent(parentId: string) {
  return axios.post(
    `${MEDICAL_MODELS_SERVICE_BASE_URL}/documents/new?parentId=${parentId}`
  );
}

export function getAllNavigation() {
  return axios.get(
    `${MEDICAL_MODELS_SERVICE_BASE_URL}/documents/all/navigation`
  );
}

export function deleteCommentById(commentId: string) {
  return axios.delete(
    `${MEDICAL_MODELS_SERVICE_BASE_URL}/comments/${commentId}`
  );
}

export function getCommentsForDocument(documentId: string) {
  return axios.get(
    `${MEDICAL_MODELS_SERVICE_BASE_URL}/comments/documents/${documentId}`
  );
}
