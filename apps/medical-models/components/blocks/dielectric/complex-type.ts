...
export function isComplex(n: MathType): n is Complex {
  return (
    isMathScalarType(n) && isNotNumberOrBigInt(n) && hasComplexProperties(n)
  );
}
...
function hasComplexProperties(
  n: Exclude<MathScalarType, number | bigint>
): n is Complex {
  return 're' in n && 'im' in n;
}

function isNotNumberOrBigInt(
  n: MathScalarType
): n is Exclude<MathScalarType, number | bigint> {
